# Persistent plugin chip bar + float minimize (CloudTAK plugins)

Plugin-only pattern (no CloudTAK core changes) for:

1. A **permanent launcher chip** on the map plugin chip bar (open your UI anytime), and
...
| Keep the chip compact (icon + short label) | Tall padding that overflows the 60px bar |
...
```text
[Plugin A enable] --> chip A beside DataSync
...
| API | Role |
|-----|------|
| `api.bottomBar.add` / `remove` | Permanent (or temporary) chips on the map plugin bar |
...
| `*TaskbarChip.vue` | Permanent chip → `openDesktopPane()` |
...
- **Shared chip bar.** Every plugin’s chip is always visible to everyone; keep labels short.
...
- **Mobile:** no map chip-bar float pattern; use menu route / `MenuTemplate`.
